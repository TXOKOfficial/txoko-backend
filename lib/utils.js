...
// ── Form details ──────────────────────────────────────
// Optional fields sent by the Framer forms (access request and consultation),
// with the label shown in the owner's email and the maximum length accepted.
...
// Cleaned optional fields from a form body; empty ones are left out.
export function readFormDetails(body) {
  const details = {};
  ...
  if (visitorMessage) details.message = visitorMessage;
  return details;
}

// The card in the owner's emails with everything the visitor typed, so the
// owner can decide without having to write back and ask.
export function detailsCardHtml(entry) {
  const detailRows = DETAIL_FIELDS.filter((field) => entry[field.key])
    .map(
      (field) =>
        `<tr>
          <td style="padding: 2px 12px 2px 0; color: #8a8378; font-size: 13px; white-space: nowrap;">${field.label}</td>
          <td style="padding: 2px 0; font-size: 14px;">${escapeHtml(entry[field.key])}</td>
        </tr>`
    )
    .join("");

  const messageBlock = entry.message
    ? `<p style="font-size: 14px; line-height: 1.7; color: #444; margin: 20px 0 0; padding-left: 14px; border-left: 2px solid #e2ddd4; font-style: italic;">${escapeHtml(entry.message)}</p>`
    : "";
  ...
  return `
    <div style="background: #f7f5f2; padding: 20px 24px; border-left: 3px solid #b8963e;">
      <p style="margin: 0 0 12px; font-size: 16px;">
        ...
        <a href="mailto:${escapeHtml(entry.email)}" style="color: #6b6255; font-size: 14px;">${escapeHtml(entry.email)}</a>
      </p>
...
// Short-lived lock, so a double click on Approve or Decline cannot run the
// action twice in parallel. Returns true when this call got the lock.
export async function acquireLock(key, seconds = 60) {
